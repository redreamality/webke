...
# FIXME: 修改了Position Embedding的计算，主要想把Layout Embbedding特征和Position相加
class PositionEmbedding(Layer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        #self.x0_emb = Embedding(3000, output_dim)
        #self.x1_emb = Embedding(3000, output_dim)
        #self.y0_emb = Embedding(5000, output_dim)
        #self.y1_emb = Embedding(5000, output_dim)
    ...
    def build(self, input_shape):
        ...
        self.emb_x0 = self.add_weight(
            name='x0_embeddings',
            shape=(3000, self.output_dim),
            initializer=self.embeddings_initializer
        )
        self.emb_x1 = self.add_weight(
            name='x1_embeddings',
            shape=(3000, self.output_dim),
            initializer=self.embeddings_initializer
        )
        self.emb_y0 = self.add_weight(
            name='y0_embeddings',
            shape=(5000, self.output_dim),
            initializer=self.embeddings_initializer
        )
        self.emb_y1 = self.add_weight(
            name='y1_embeddings',
            shape=(5000, self.output_dim),
            initializer=self.embeddings_initializer
        )
    ...
    def call(self, inputs):
        ...
        if self.custom_position_ids:
            ...
            inputs, x0_ids, y0_ids, x1_ids, y1_ids = inputs
            if 'int' not in K.dtype(x0_ids):
                x0_ids = K.cast(x0_ids, 'int32')
            if 'int' not in K.dtype(x1_ids):
                x1_ids = K.cast(x1_ids, 'int32')
            if 'int' not in K.dtype(y0_ids):
                y0_ids = K.cast(y0_ids, 'int32')
            if 'int' not in K.dtype(y1_ids):
                y1_ids = K.cast(y1_ids, 'int32')
        ...
        if self.hierarchical:
            ...
            embeddings_y = K.gather(embeddings, position_ids % self.input_dim)
            embeddings = alpha * embeddings_x + (1 - alpha) * embeddings_y
        #print(K.gather(self.emb_x0,x0_ids))
        if self.custom_position_ids:
            embeddings = K.gather(self.embeddings, position_ids) + \
                            K.gather(self.emb_x0,x0_ids) + \
                            K.gather(self.emb_x1,x1_ids) + \
                            K.gather(self.emb_y0,y0_ids) + \
                            K.gather(self.emb_y1,y1_ids)
